...
public class PPatchesConfig {
    ...
    @Config.Comment({
            ...
            "This is unlikely to noticeably affect performance by itself, however it may allow the JIT compiler to optimize more aggressively when combined with the forge.optimizeEventBusDispatch module.",
    })
    ...
    @Config.Comment({
            ...
            "This uses INVOKEDYNAMIC to generate classes at runtime, resulting in code equivalent to a monomorphic virtual call to a method containing a sequence of static calls to event handler methods, rather than an interface call to get the listener list followed by two interface calls for each handler.",
            "This can give significant (~10-15%) performance improvements when many events are being fired, especially in situations where there are many listeners for a single event type.",
    })
    @ModuleDescriptor(
            registerPhase = PPatchesBootstrap.Phase.PREINIT,
